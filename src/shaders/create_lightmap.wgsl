...
fn apply_occlusion(shadow: vec3<f32>, index: u32, occ: OccRes, pos: vec2<f32>) -> vec3<f32> {
    ...
    if occ.occluded && index != 0u {
        ...
        if occ.occluded_left && occ.occluded_right {
            if orientation(pos, occ.left_point, occ.right_point) < 0 {
        ...
        let smooth1 = smoothstep(0, 1, multi);
        return shadow_blend(shadow, poly_occluders[index].color, poly_occluders[index].opacity * smooth1);
    }
    else {
        return shadow;
    }
...
fn res_full_occlusion() -> OccRes {
    return OccRes(true, true, 1.0, vec2<f32>(0.0), true, 1.0, vec2<f32>(0.0), false, false, true);
}

fn res_no_occlusion() -> OccRes {
    return OccRes(false, false, 0.0, vec2<f32>(0.0), false, 0.0, vec2<f32>(0.0), false, false, true);
}
...
fn poly_check(pos: vec2f, index: u32, term: u32, rev: u32, min_v: u32, split: u32, length: u32) -> OccRes {
    ...
    if !out_of_bounds {
        if rev == 0 {
            ...
            let v1 = vertices[start - u32(maybe_prev) + select(0, occluder.n_vertices, start - u32(maybe_prev) < occluder.start_vertex)];
            let v2 = vertices[start - u32(maybe_prev) - 1 + select(0, occluder.n_vertices, start - u32(maybe_prev) - 1 < occluder.start_vertex)];
...
fn get_softness_multi(pos: vec2<f32>, extreme_left: vec2<f32>, prev_extreme_left: vec2<f32>, extreme_right: vec2<f32>, prev_extreme_right: vec2<f32>, prev: vec2<f32>, next: vec2<f32>, out_of_bounds: bool, term: u32) -> OccRes {
    ...
    return OccRes(left || right || !out_of_bounds || maybe_left || maybe_right, left, left_multi, extreme_left, right, right_multi, extreme_right, maybe_left, maybe_right, !out_of_bounds);   
}
...
fn vertex_reverse(start: u32, index: u32, start_vertex: u32, n_vertices: u32) -> vec2<f32> {
    if start - index < start_vertex {
        return vertices[start - index + n_vertices];
    }
    return vertices[start - index];
} 
...
fn bs_vertex_reverse(angle: f32, start: u32, length: u32, term: u32, start_vertex: u32, n_vertices: u32) -> i32 {
    ...
    if angle < angle_term(vertex_reverse(start, u32(low), start_vertex, n_vertices), u32(low), length, term) {
        return -1;
    }

    if angle >= angle_term(vertex_reverse(start, u32(high), start_vertex, n_vertices), u32(high), length, term) {
        return high + 1;
    }

    while (low <= high) {
        let mid = low + (high - low + 1) / 2;
        let val = angle_term(vertex_reverse(start, u32(mid), start_vertex, n_vertices), u32(mid), length, term);
